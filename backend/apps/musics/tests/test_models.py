...
class TestMusicModel:
    def test_get_vide_content_type_id(self, create_music):
        # Get from db
        music_content_type_id = ContentType.objects.get_for_model(type(create_music)).id

        # Assert with the method
        assert (
            get_content_type_model(model=type(create_music)).id == music_content_type_id
        )

    def test_get_music_content_type_model(self, create_music):
        music_content_type = ContentType.objects.get_for_model(type(create_music))
    ...
    def test_get_music_views(self, create_music):
        ...
        assert (
            Viewer.objects.get_count(
                content_object=create_music, channel=create_music.channel
            )
            == 0
        )
    ...
    def test_delete_votes_after_deleting_music(self, create_music):
        ...
        Vote.objects.create(user=create_music.user, content_object=create_music)
    ...
    def test_delete_comment_after_deleting_music(self, create_music):
        ...
        Comment.objects.create(
            user=create_music.user, content_object=create_music, body="Test comment"
        )
