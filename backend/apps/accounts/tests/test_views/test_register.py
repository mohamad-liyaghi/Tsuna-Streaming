...
class TestRegisterUserView:
    ...
    def test_register_authenticated(self, api_client, create_superuser):
        api_client.force_authenticate(user=create_superuser)
        response = api_client.post(self.url_path, self.data, format="json")
        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_register_duplicate_user(self, api_client, create_active_user):
        """
    ...
    def test_duplicate_deactive_user_email(self, api_client, create_deactive_user):
        """
